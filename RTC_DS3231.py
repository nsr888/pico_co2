...
class RTC:
    ...
    def __init__(self, i2c: machine.I2C, address=0x68, register=0x00):
        ...
        self.i2c = i2c  # i2c object has to be passed to the RTC class
    ...
    def DS3231_ReadTime(self, mode=0):
        try:
            ...
            hour = self.pre_zero(
                self.bcd2bin(buffer[2])
            )  # Convert bcd to bin and add a "0" if necessary
            minute = self.pre_zero(
                self.bcd2bin(buffer[1])
            )  # Convert bcd to bin and add a "0" if necessary
            second = self.pre_zero(
                self.bcd2bin(buffer[0])
            )  # Convert bcd to bin and add a "0" if necessary
            ...
            if (
                ...
                return time_string
            if mode == 2:
                # Example: 2025-06-17T21:10:00
                iso_string = f"{year:04d}-{month:02d}-{day:02d}T{hour:02d}:{minute:02d}:{second:02d}"
                return iso_string
